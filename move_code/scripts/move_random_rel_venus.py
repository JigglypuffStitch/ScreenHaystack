...
# ----------------------------
# Prompt (UI-Venus bbox output)
# ----------------------------
SYSTEM_PROMPT = "Outline the position corresponding to the instruction: {}. The output should be only [x1,y1,x2,y2]."
...
DEFAULT_REGION_BBOX_PATH = "/pasteur/u/andy0207/chenyue/gta_blind_zero_region_bounds_only.jsonl"
...
def extract_coordinates(
    ...
    *,
    ...
    coord_output_mode:
      ...
      - venus_bbox: model outputs [x1,y1,x2,y2] in UI-Venus processed image coordinates.
...
def bbox_parts_respect_min_region_gap(
    bbox_parts: List[Tuple[float, float, float, float]],
    region_boxes_xyxy: List[Tuple[float, float, float, float]],
    *,
    dilate_x: float,
    ...
    return (not bbox_parts_intersect_any_region(
        bbox_parts,
        region_boxes_xyxy,
        dilate_x=eff_dx,
        dilate_y=eff_dy,
...
def pick_best_shift_wrap_style(
    *,
    gt_xyxy: Tuple[float, float, float, float],
    W: int,
    H: int,
    ...
    dilate_x: float,
    dilate_y: float,
    min_region_gap_x: float,
    min_region_gap_y: float,
    placement_canvas_w: int,
    placement_canvas_h: int,
    edge_margin: int,
    style: str,
    ...
    W = int(W)
    H = int(H)
    placement_canvas_w = int(placement_canvas_w)
    placement_canvas_h = int(placement_canvas_h)
    edge_margin = int(edge_margin)
    min_region_gap_x = max(0.0, float(min_region_gap_x))
    min_region_gap_y = max(0.0, float(min_region_gap_y))
    ...
    for dx0 in sorted(dx_vals):
        for dy0 in sorted(dy_vals):
            ...
            if not bbox_parts_respect_min_region_gap(
                ...
                min_gap_x=min_region_gap_x,
                min_gap_y=min_region_gap_y,
            ):
                continue

            min_region_dist = min_distance_bbox_parts_to_regions(
                gt_parts,
                region_boxes_proc,
                dilate_x=float(dilate_x),
                dilate_y=float(dilate_y),
            ...
            if best is None or key < best["key"]:
                best = {
                    ...
                    "min_region_distance": float(min_region_dist),
                    "min_region_gap_x": float(min_region_gap_x),
                    "min_region_gap_y": float(min_region_gap_y),
                    "style_penalty": float(style_pen),
                    "bbox_after_shift_parts": [[float(a), float(b), float(c), float(d)] for (a, b, c, d) in gt_parts],
                    "bbox_within_placement_canvas": True,
                    "bbox_respects_edge_margin": True,
                    "bbox_respects_min_region_gap": True,
                    "key": key,
                }
...
def build_multi_run_shift_plans(
    *,
    ...
    debug: Dict[str, Any] = {
        ...
        "dilate_xy": [float(dilate_x), float(dilate_y)],
        "shift_mode": "wrap",
        "rule": (
            "direct non-zero wrap shift; bbox must remain complete (not split across wrap boundary), "
            "must stay outside all regions with the required min_region_gap safety band, "
            "must be fully within the original image while keeping edge_margin to all image borders, "
            "and candidates farther away from regions are preferred"
        ),
        "placement_canvas_wh": [int(placement_canvas_w), int(placement_canvas_h)],
        "edge_margin_px": int(edge_margin),
        "min_region_gap_xy": [float(min_region_gap_x), float(min_region_gap_y)],
    ...
    for run_i, style in enumerate(styles, 1):
        cand = pick_best_shift_wrap_style(
            gt_xyxy=gt_xyxy,
            W=W,
            H=H,
            budget_px=budget_px,
        ...
        if cand is None:
            plans.append({
                "run": int(run_i),
                "skipped": True,
                "reason": "no_feasible_nonzero_wrap_shift_with_complete_bbox_under_constraints_or_min_region_gap",
                "style": str(style),
            })
            continue
        ...
        plans.append({
            ...
            "applied_fraction": "full",
            "applied_fraction_human": "full",
            "fraction_mode": "direct_wrap_shift",
            "filled_px": int(cand["filled_px"]),
            "filled_ratio": (0.0 if (W * H) == 0 else float(cand["filled_px"]) / float(W * H)),
            "bbox_after_shift_parts": cand.get("bbox_after_shift_parts", None),
            "min_region_distance": float(cand.get("min_region_distance", 0.0)),
            "min_region_gap_x": float(cand.get("min_region_gap_x", min_region_gap_x)),
            "min_region_gap_y": float(cand.get("min_region_gap_y", min_region_gap_y)),
            "bbox_within_placement_canvas": bool(cand.get("bbox_within_placement_canvas", False)),
            "bbox_respects_edge_margin": bool(cand.get("bbox_respects_edge_margin", False)),
            "bbox_respects_min_region_gap": bool(cand.get("bbox_respects_min_region_gap", False)),
        })
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description=(
            ...
            "and inclusionAI/UI-Venus-Ground-7B bbox outputs."
        )
    )
    ...
    # ---- dilation ----
    ap.add_argument("--dilate_px", type=float, default=0.0)
    ...
    ap.add_argument(
        ...
        type=float,
        default=99.0,
        help="Max seam-area pixels percent of original when using wrap shift.",
    )
    ap.add_argument("--num_crop_runs", type=int, default=5, help="Number of runs (shift plans).")

    # ---- original-image edge constraints ----
    ap.add_argument("--edge_margin_px", type=int, default=350)
    ...
    # resolve dilation
    px_x = args.dilate_px if args.dilate_px_x is None else args.dilate_px_x
    px_y = args.dilate_px if args.dilate_px_y is None else args.dilate_px_y
    if px_x < 0 or px_y < 0:
        raise ValueError(f"dilate_px must be non-negative. Got px_x={px_x}, px_y={px_y}")

    min_region_gap_x = args.min_region_gap_px if args.min_region_gap_px_x is None else args.min_region_gap_px_x
    min_region_gap_y = args.min_region_gap_px if args.min_region_gap_px_y is None else args.min_region_gap_px_y
    if min_region_gap_x < 0 or min_region_gap_y < 0:
        raise ValueError(
            f"min_region_gap_px must be non-negative. Got min_region_gap_x={min_region_gap_x}, min_region_gap_y={min_region_gap_y}"
        )
    ...
    rank0_print(rank, "[Mode] ✅ SHIFT-ONLY fixed canvas: WRAP-AROUND shift; NO crop; must move.")
    ...
    rank0_print(
        rank,
        f"[Shift] direct non-zero WRAP shift; bbox must remain complete, stay outside all regions with "
        f"min_region_gap=({min_region_gap_x},{min_region_gap_y}), stay fully within the ORIGINAL image and keep >= "
        f"{args.edge_margin_px}px from original-image borders; among feasible shifts prefer those farthest from regions; "
        f"dilate=({px_x},{px_y})",
    )
    rank0_print(rank, f"[AspectFix] aspect_ratio_limit={args.aspect_ratio_limit} (<200), will pad if too skinny")
    rank0_print(rank, f"[Parse] coord_output_mode={args.coord_output_mode} (venus_bbox / auto / pixel / norm1000 / rel01)")
    ...
    with open(output_path_rank, out_mode, encoding="utf-8") as fout:
        for list_idx in pbar:
            ...
            u0, region_bboxes_abs_orig, region_boxes_proc = get_union_for_original_image(W, H)
            ...
            lr_crop_search: Dict[str, Any] = {
                ...
                "_placement": {
                    "canvas_type": "original_image",
                    "canvas_w": int(W),
                    "canvas_h": int(H),
                    "edge_margin_px": int(args.edge_margin_px),
                    "used": (sample_gt_type != "negative"),
                },
            }
            ...
            def _eval_one_shift(
                ...
                trial["min_region_gap_x"] = float(plan.get("min_region_gap_x", min_region_gap_x))
                trial["min_region_gap_y"] = float(plan.get("min_region_gap_y", min_region_gap_y))
                trial["bbox_within_placement_canvas"] = bool(plan.get("bbox_within_placement_canvas", False))
                trial["bbox_respects_edge_margin"] = bool(plan.get("bbox_respects_edge_margin", False))
                trial["bbox_respects_min_region_gap"] = bool(plan.get("bbox_respects_min_region_gap", False))
                ...
                shifted_bbox_within_placement_runtime = None
                if gt_parts is None or len(gt_parts) == 0:
                    ...
                    if len(gt_parts) == 1:
                        ...
                        shifted_bbox_within_placement_runtime = False
                ...
                venus_instruction = instruction[:-1] if instruction.endswith(".") else instruction
                user_prompt = SYSTEM_PROMPT.format(venus_instruction).strip()
                ...
                if infer_result is not None:
                    ...
                    source_size = infer_result.get("source_size", None)
                else:
                    source_size = None
                    try:
                        messages = [{
                    ...
                    source_size = get_venus_processed_size(inputs, 0)
                ...
                if gt_xyxy0 is not None:
                    ...
                    bbox_respects_min_region_gap_runtime = bbox_parts_respect_min_region_gap(
                        gt_parts_dbg,
                        region_boxes_proc,
                        dilate_x=float(px_x),
                        dilate_y=float(px_y),
                        min_gap_x=float(min_region_gap_x),
                        min_gap_y=float(min_region_gap_y),
                    )
                    ...
                    bbox_respects_min_region_gap_runtime = None
                ...
                trial["_debug"] = {
                    ...
                    "bbox_respects_min_region_gap_runtime": bbox_respects_min_region_gap_runtime,
                    "center_xy_before": center_xy_before,
                    "center_xy_after_wrap": center_xy_after,
                    "placement_canvas_wh": [int(W), int(H)],
                    "edge_margin_px": int(args.edge_margin_px),
                    "min_region_gap_xy": [float(min_region_gap_x), float(min_region_gap_y)],
                    "placement_constraints_used": (sample_gt_type != "negative"),
                    "bbox_within_placement_canvas_runtime": shifted_bbox_within_placement_runtime,
                    "region_vis_count_model": int(len(region_boxes_for_model)),
                    "region_proc_count": int(len(region_boxes_proc)),
                    "shift_mode": "wrap",
                    "edge_constraint_canvas_type": "original_image",
                }
            ...
            def _infer_deferred_shift_trials(prepared_trials: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
                ...
                def _infer_one(payload: Dict[str, Any]) -> Dict[str, Any]:
                    try:
                        ...
                        return {"raw": "", "prompt_len": 0, "source_size": None, "error": str(e)}
                ...
                for start in range(0, len(prepared_trials), batch_size):
                    ...
                    try:
                        ...
                        inputs.pop("token_type_ids", None)
                        inputs = {k: (v.to(device) if torch.is_tensor(v) else v) for k, v in inputs.items()}
                        source_sizes = [get_venus_processed_size(inputs, i) for i in range(len(chunk))]
            ...
            for ri, plan, run_tag in run_items:
                ...
                multi_run_choices.append({
                    ...
                    "min_region_distance": float(plan.get("min_region_distance", 0.0)),
                    "min_region_gap_x": float(plan.get("min_region_gap_x", min_region_gap_x)),
                    "min_region_gap_y": float(plan.get("min_region_gap_y", min_region_gap_y)),
                    "bbox_within_placement_canvas": bool(plan.get("bbox_within_placement_canvas", False)),
                    "bbox_respects_edge_margin": bool(plan.get("bbox_respects_edge_margin", False)),
                    "bbox_respects_min_region_gap": bool(plan.get("bbox_respects_min_region_gap", False)),
                })
            ...
            out = {
                ...
                "_debug": {
                    ...
                    "dilate_px_xy": [float(px_x), float(px_y)],
                    "min_region_gap_xy": [float(min_region_gap_x), float(min_region_gap_y)],
                    "placement_canvas_w": int(W),
                    "placement_canvas_h": int(H),
                    "edge_margin_px": int(args.edge_margin_px),
                    "movement_rule": (
                        "shift-only fixed canvas; WRAP-AROUND shift; must use non-zero shift; "
                        "bbox must remain complete (not split by wrap), shifted bbox must stay outside all regions "
                        "with the required min_region_gap, must stay fully within the original image with the required "
                        "edge_margin, and among feasible candidates prefer those farthest from regions"
                    ),
                    "model_type": str(mt),
                    "edge_constraint_canvas_type": "original_image",
                },
