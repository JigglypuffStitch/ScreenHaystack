...
def extract_coordinates(
    ...
    *,
    ...
    coord_output_mode controls how the parsed coordinate pair is interpreted:
      ...
      - venus_bbox: raw box is [x1,y1,x2,y2] in UI-Venus processed image coordinates.
...
def run_vlm_once(
    *,
    ...
    text_prompt = SYSTEM_PROMPT.format(venus_instruction)
    messages = [
        {
            "role": "user",
            "content": [
...
def main() -> None:
    ...
    # ---- model ----
    ap.add_argument("--model_path", type=str, default="inclusionAI/UI-Venus-Ground-7B")
    ...
    model_kwargs = dict(torch_dtype=torch.bfloat16, trust_remote_code=True, low_cpu_mem_usage=True)
    if args.attn_impl:
